...
semivol=0.3 #volume settings
full=1
soft=0.5

define :ldc do |dlist| #used to calculate part lengths in crotchets to make it easy to set delays
  dur=0
  dlist.each do |d|
    dur+=d
  end
...
cor4=cor[0..4]
#puts cor4 #for debugging

lhn=[co,:r,co,:r]+cor*7+cor4*2+[:r]
dor=[qt,qt,qt,q,q,q,q]
dor4=dor[0..4]
lhd=[q,q,q,q]+dor*7+dor4*2+[q]
lhn=lhn.ring #put into rings to allow multiple indexed plays
lhd=lhd.ring
...
finaleld=[q,q+7*c]
puts "rh4"
puts rhn4.length #length and duration checks for debugging and setting intersection delays
puts rhd4.length
puts "lh4"
puts lhn4.length
puts lhd4.length
...
with_fx :reverb,room: 0.8 do
  2.times do #play twice through. Can set higher if desired or loop for continuous
    ...
    sleep 48*c
    live_loop :rh4 do #make more sustained from here on in, using sustain and release
      tick
    ...
    live_loop :rh4b do  #extra two notes in rh part
      tick
